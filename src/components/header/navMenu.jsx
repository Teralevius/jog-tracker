  ...
  return (
    <div className='navMenu'>
      { width > 768 ? items.map((current, index) => (
        <NavLink activeClassName='is-active' className='navItem' key={index}
          to={current.path}>{current.text}</NavLink>)) :
          <img src={menu} alt='Menu' />
      }
      <div className='imgFilterContainer'>
        <img src={ filterIconState ? filterOn : filterOff }
          onClick={filterOnClick} alt='Filter' />
      </div>
    </div>
  );
};
